...
export default function PetDetailPage({ params }: { params: { id: string } }) {
  ...
  const pet_id = parseInt(params.id);
  ...
  useEffect(() => {
    const fetchPet = async () => {
      try {
        ...
        await checkFavoriteStatus(pet_id);
    ...
    fetchPet();
  }, [params.id, pet_id, checkFavoriteStatus]);
            ...
            <CardContent>
              <div className="space-y-6">
                ...
                <div className="pt-4 flex justify-between">
                  ...
                  {!isShelterOwner && (
                    <Button 
                      className={`flex items-center space-x-2 ${favoriteStatus[pet_id] ? 'bg-red-500 hover:bg-red-600' : ''}`}
                      onClick={handleToggleFavorite}
                    >
                      <Heart className="h-4 w-4" fill={favoriteStatus[pet_id] ? "currentColor" : "none"} />
                      <span>{favoriteStatus[pet_id] ? "Remove from Favorites" : "Add to Favorites"}</span>
                    </Button>
                  )}
                </div>
              </div>
